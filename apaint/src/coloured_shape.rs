...
pub struct HueWheelNew<F: ColourComponent + ShapeConsts> {
    shapes: Vec<ColouredShape<F>>,
    target: Option<ColouredShape<F>>,
}

impl<F: ColourComponent + ShapeConsts> Graticule<F> for HueWheelNew<F> {}

impl<F: ColourComponent + ShapeConsts> HueWheelNew<F> {
    pub fn new() -> Self {
        Self {
            shapes: vec![],
            target: None,
